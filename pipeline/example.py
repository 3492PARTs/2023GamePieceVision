...
####IMPORTANT NOTES####
# under __find_contours, remove the first variable from im2, contours ,hierarchy =cv2.findContours(input, mode=mode, method=method), a.k.a. remove im2.
# this is because the current cv2 that grip uses is slightly outdated, so the syntax have changed and the first variable is no longer supported.
# besides, the first variable isn't actually used for anything, it was just part of the original version of cv.
# fixing legacy code is still faster than creating this from scratch though :D

#optionally, under __filter_contours, before the return statement you can add print("worked") to see if the pipeline worked.
#this might cause too much clutter, because you'll know if it works when you get a giant array.

#TO DO: remember to resize the feed so that distance estimation is easy.
#######################
...
class ExamplePipeline:
    ...
    def __init__(self):
        ...
        self.__hsv_threshold_0_hue = [0.0, 63.37500684418975]
        self.__hsv_threshold_0_saturation = [21.610169491525422, 157.27272727272725]
        self.__hsv_threshold_0_value = [43.220338983050844, 223.1818181818182]

        self.hsv_threshold_0_output = None


        self.__hsv_threshold_1_hue = [92.06185467108598, 152.522940347999]
        self.__hsv_threshold_1_saturation = [136.05251392106655, 255.0]
